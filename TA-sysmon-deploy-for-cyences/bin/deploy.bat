...
if "%CHECK_SYSMON_RUNNIG%" == "1" (
    ...
    COPY /z /y "%SPLUNKPATH%\etc\apps\TA-sysmon-deploy-for-cyences\bin\sysmonconfig.xml" "C:\Windows\"
    ECHO %DATE%-%TIME% Installing Sysmon
    "%SPLUNKPATH%\etc\apps\TA-sysmon-deploy-for-cyences\bin\Sysmon.exe" /accepteula -i C:\Windows\sysmonconfig.xml | Find /c "Sysmon installed" 1>NUL
